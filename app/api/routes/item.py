...
def list_item(
    ...
    items, total = service_list_item(session, token, skip, limit)
    return {"items": items, "total": total, "skip": skip, "limit": limit}


@router.post("/", response_model=ItemResponse)
def register(session: SessionDep, token: Token, item_in: ItemCreate):
    return create_item(session, item_in, token)
...
def update_item(
    ...
    return service_update_item(session, item, item_in)
...
def delete_item(
    ...
    return service_delete_item(session, item)
...
def update_item_status(
    ...
    return service_change_status(session, item, status_update.status)
